...
void IRsend::begin() {
#if !defined(UNIT_TEST) && IRREMOTE_RA4M2_USE_GPT_SEND
  (void)irremote_ra4m2_gpt_carrier_begin(IRpin, outputOn);
#else
#ifndef UNIT_TEST
...
void IRsend::ledOff() {
#if !defined(UNIT_TEST) && IRREMOTE_RA4M2_USE_GPT_SEND
  if (irremote_ra4m2_gpt_carrier_handles_pin(IRpin)) {
    irremote_ra4m2_gpt_carrier_mark_stop();
    return;
  }
#endif
#ifndef UNIT_TEST
...
#if !defined(UNIT_TEST) && IRREMOTE_RA4M2_USE_GPT_SEND
  if (irremote_ra4m2_gpt_carrier_handles_pin(IRpin)) {
    (void)irremote_ra4m2_gpt_carrier_mark_start();
    return;
  }
#endif
#ifndef UNIT_TEST
...
void IRsend::enableIROut(uint32_t freq, uint8_t duty) {
  ...
  if (modulation && (_dutycycle < kDutyMax)) {
    (void)irremote_ra4m2_gpt_carrier_configure(IRpin, outputOn, freq,
                                               _dutycycle);
  }
#endif
}
...
#if !defined(UNIT_TEST) && IRREMOTE_RA4M2_USE_GPT_SEND
  if (modulation && (_dutycycle < kDutyMax)) {
    if (irremote_ra4m2_gpt_carrier_ready() &&
        irremote_ra4m2_gpt_carrier_mark_start()) {
      _delayMicroseconds(usec);
      irremote_ra4m2_gpt_carrier_mark_stop();
      return irremote_ra4m2_gpt_carrier_estimate_pulses(usec);
    }
